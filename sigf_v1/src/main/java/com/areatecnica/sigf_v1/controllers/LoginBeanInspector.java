 ...
 * @author Ian Franco
 */
@Named(value = "loginBeanInspector")
@SessionScoped
public class LoginBeanInspector implements Serializable {
        ...
        if (this.usuario != null) {
            ...
            switch (this.rol.getIdRol()) {                
                case 2:
                    ...
                    message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Bienvenido " + this.usuario.getRol().getNombreRol(), this.usuario.getNombres());
                    path = MyUtil.basePathLogin() + "views/indexInspector.xhtml";
                    break;                
                default:
                    loggedIn = false;
                    message = new FacesMessage(FacesMessage.SEVERITY_WARN, "Loggin Error", "USUARIO SIN ACCESO AL SISTEMA DE INSPECTORES");
                    break;
            }
